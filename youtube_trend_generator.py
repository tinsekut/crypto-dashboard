...
def extract_title_patterns(titles: list[str]) -> dict:
    """Detect common viral title patterns across a list of titles."""
    patterns = {
        "numbers": 0,
        "questions": 0,
        "caps_words": 0,
...
def top_keywords(titles: list[str], n: int = 20) -> list[tuple[str, int]]:
    """Extract most frequent meaningful words from titles."""
    stop = {
...
def generate_titles_from_patterns(
    ...
    """Rule-based title generator when Claude API is not available."""
    ...
    with tab3:
        ...
        if st.button("Generate Ideas", type="primary"):
            ...
            angle_tips = {
                "Numbers": (
                    patterns["numbers"] > 50,
                    "Use numbered lists in your title (e.g. '7 Ways to…')",
                ),
                "Questions": (
                    patterns["questions"] > 30,
                    "Frame your title as a question to spark curiosity",
                ),
                "ALL CAPS words": (
                    patterns["caps_words"] > 40,
                    "Capitalise 1-2 power words to add emphasis",
                ),
                "Colons": (
                    patterns["colons"] > 40,
                    "Use a colon to add subtitle context (e.g. 'Topic: Why It Matters')",
                ),
                "Parentheses": (
                    patterns["parentheses"] > 30,
                    "Add context in brackets (e.g. '(My Honest Review)')",
                ),
            }
...
# ── Setup guide (shown before first fetch) ──────────────────────────────────
